...
//                    this.logger.debug(this.threadId + ' browser version : ' + brws_version);
    ...
    this.loginOnSite = async function() {
        if (typeof this.config['identity'] !== undefined) {
            ...
            if (typeof this.config['emulateIphone'] !== undefined && this.config['emulateIphone'] == true) {
            ...
            try {
                let cssSelectorDidomiAgree='';
                if (typeof this.config['hasConsentButton'] === undefined || this.config['hasConsentButton'] != true) {
                    cssSelectorDidomiAgree = '#didomi-notice-disagree-button';
                } else {
                    cssSelectorDidomiAgree = '#didomi-notice-agree-button';
                }
                this.logger.debug(this.threadId + ' Waiting for the didomi agree button ...');
                const btnDidomiAgree = await this.page.waitForSelector(cssSelectorDidomiAgree, {timeout: 20000 });
                this.logger.debug(this.threadId + ' Clicking the didomi agree button ...');
                await btnDidomiAgree.click();
            } catch(e) {
                this.logger.error(this.threadId + 'Unable to find the didomi agree button'+ e.stack);
            }
            ...
            await this.page.click(cssSelectorButton, { delay: 100 }).catch(() => {
                ...
                this.logger.error(this.threadId + " Process exitLoginPageError event button to validate password not found " + exit_code + ' ' + e.stack);
                process.stderr.write(e.stack);
                process.exit(exit_code);
            });
    ...
    this.gotoUrl = async function() {
        ...
        this.logger.info(`${this.threadId} Page load took: ${Date.now() - tic}ms`);
    ...
    this.checkHAR_genScreenshot = async function() {
        ...
        await this.page.setViewport({ width: this.config['page_viewport']['width'], height: viewportIncr });
    ...
    this.closeBrowser = async function() {
        await this.browser.close()
            .then(() => {
                ...
                e.stack = e.stack + "\n" + startStack.substring(startStack.indexOf("\n") + 1);
                this.logger.error(this.threadId + " Browser already closed " + e.stack)
            });
    }
}
