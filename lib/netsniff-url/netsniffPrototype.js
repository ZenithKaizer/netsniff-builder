...
function netsniffPrototype(config, filename, threadId, browserLessURL) {
    ...
    // this.filename = filename.replace(/:\/\/|\//gi, '_') + this.threadId;
        ...
        if (this.config['identity'] != undefined) {
            ...
            try {
                let response = await this.page.waitForResponse(
                    response => response.url() === url + '/front/password' && response.status() === 200
                );
                let response_json = await response.json();

                // Vérification de l'authentification
                if ('return_url' in response_json) {
...
*/
    ...
    this.checkHAR_genScreenshot = async function() {
        ...
        this.logger.info(this.threadId + ' Convert HAR File as object');
